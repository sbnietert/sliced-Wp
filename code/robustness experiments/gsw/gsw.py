...
class GSW():
    ...
    def gsw(self,X,Y,theta=None,p=2.0):
        '''
        ...
        return torch.mean(torch.abs(Xslices_sorted-Yslices_sorted)**p)**(1/p)

    def max_gsw(self,X,Y,iterations=50,lr=1e-4,rand_init=True,p=2.0):
        N,dn = X.shape
        M,dm = Y.shape
        device = self.device
        assert dn==dm and M==N
#         if self.theta is None:
        if self.ftype=='linear':
            if rand_init:
                ...
                theta = X.mean(dim=0)-Y.mean(dim=0)
...
#             iterations=iterations
        ...
        for i in range(iterations):
            optimizer.zero_grad()
            loss=-self.gsw(X.to(self.device),Y.to(self.device),self.theta.to(self.device),p=p)
